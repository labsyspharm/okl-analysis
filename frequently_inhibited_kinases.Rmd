...
most_inhibited_inhibitors <- most_inhibited %>%
  group_by(cutoff) %>%
  slice_max(n_inhibitors, n = TOP_N, with_ties = TRUE) %>%
  ungroup() %>%
  inner_join(
    okl_kds_dedup,
...
most_inhibited_raw_no <- okl_kds_dedup_no %>%
  group_by(hgnc_symbol, `DiscoveRx Gene Symbol`) %>%
  summarize(
    n_inhibitors = map_int(
      set_names(CUTOFFS),
...
most_targets_raw_no <- okl_kds_dedup_no %>%
  group_by(lspci_id, hmsl_id, library, name) %>%
  summarize(
    n_targets = map_int(
      set_names(CUTOFFS),
...
mutant_hm_data <- tibble(
  ...
  mutate(
    hm = map(
      cutoff,
      \(cu) {
        mat <- okl_kds_dedup_no %>%
          filter(`DiscoveRx Gene Symbol` %in% mutant_panel$`DiscoveRx Gene Symbol`) %>%
          transmute(`DiscoveRx Gene Symbol`, lspci_id, kd = log10(kd)) %>%
          pivot_wider(names_from = lspci_id, values_from = kd) %>%
        ...
        Heatmap(
          ...
          cluster_rows = \(x) cluster_fun_eucl(x, sample_in_col = TRUE),
...
pwalk(
  ...
  \(cutoff, hm) {
    withr::with_pdf(
      here("plots", paste0("mutant_panel_fit_kd_heatmap_cutoff_", cutoff, ".pdf")),
    ...
    hm = map2(
      ...
      \(cu, g) {
        ...
        mat <- okl_kds_dedup_no %>%
          ...
          transmute(`DiscoveRx Gene Symbol`, lspci_id, across(kd, \(x) log10(pmin(x, 10000)))) %>%
          pivot_wider(names_from = lspci_id, values_from = kd) %>%
          column_to_rownames("DiscoveRx Gene Symbol") %>%
          as.matrix()
        # browser()
        ...
        Heatmap(
          ...
          # show_column_names = FALSE,
          width = ncol(subset_mat) * unit(5, "mm"),
...
pwalk(
  ...
  \(cutoff, hgnc_symbol, hm) {
    ...
    withr::with_pdf(
      here("plots", paste0("mutant_panel_", hgnc_symbol, "_fit_kd_heatmap_cutoff_", cutoff, "_names.pdf")),

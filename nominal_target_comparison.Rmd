...
pseudo_kds_nominal_top_n <- tibble(
  top_x = seq_len(length(unique(pseudo_kds_nominal_target_sorted$`DiscoveRx Gene Symbol`)))
) %>%
  mutate(
    data = map(
      top_x,
      ~pseudo_kds_nominal_target_sorted %>%
        group_by(lspci_id) %>%
        filter(any(nominal_target)) %>%
        ungroup() %>%
        group_by(lspci_id, hmsl_id, name) %>%
        summarize(
          in_top_n = any(nominal_target[1:.x + 1]),
          .groups = "drop"
        )
    ),
    proportion_in_top_n = map_dbl(
      data,
      ~ mean(.x$in_top_n)
    )
  )

p <- pseudo_kds_nominal_top_n %>%
  ggplot(aes(top_x, proportion_in_top_n)) +
  geom_point() +
  geom_line() +
  scale_x_log10() +
  ...
  labs(
    x = "Top N targets",
    y = "Proportion of compounds with\nnominal target in top N targets"
  )
...
pseudo_kds_nominal_top_n <- tibble(
  ...
  mutate(
    data = map(
      top_x,
      ~ pseudo_kds_nominal_target_sorted %>%
        filter(lspci_id %in% nominal_targets$lspci_id) %>%
        group_by(lspci_id, hmsl_id, name) %>%
        summarize(
          in_top_n = any(nominal_target[1:.x + 1]),
          .groups = "drop"

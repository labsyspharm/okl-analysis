...
plot_single_drug <- function(drug, target) {
  ...
  misses <- qpcr_misses %>%
    rename(
      ...
      `Compound Concentration (nM)` = `Compound Conc`
    ) %>%
    filter(lspci_id == .env$lspci_id, str_detect(`DiscoveRx Gene Symbol`, target))
  p <- df %>%
    ggplot(aes(as.ordered(`Compound Concentration (nM)`), `Percent Control`)) +
      geom_quasirandom(groupOnX = TRUE) +
      geom_point(data = misses, y = 100, color = "red", size = 3, shape = 8)
...
p <- plot_single_drug("HMSL10004", "TYK2")
p
...
```{r}
single_dose_groups <- single_dose_trans %>%
  distinct(lspci_id, hmsl_id, `DiscoveRx Gene Symbol`, `Entrez Gene Symbol`, superseded) %>%
  mutate(
    dataset_version = if_else(superseded, "original", "repeat")
  ) %>%
  select(-superseded) %>%
  nest_join(
    single_dose_trans,
    keep = TRUE,
    name = "data",
    by = c("lspci_id", "hmsl_id", "DiscoveRx Gene Symbol")
  ) %>%
  mutate(
    data = map2(
      data,
...
# single_dose_classification <- qread(
#   file.path("classification", "okl_eurofins_classification_raw.qs")
# )
...
ggsave(
  "classification/discordant_vs_binding.pdf", p, width = 7, height = 7
)
...
ggsave(
  "classification/discordant_vs_binding_kinases.pdf", p, width = 7, height = 7
)

```

```{r pseudo_ic_50}
compute_pseudo_ic50 <- function(df) {
  df <- df %>%
  ...
  if (length(unique(df$below_50)) == 2 && !df$below_50[1]) {
    ...
    list(
      ...
      # Make sure all values beyond the IC50 value are below 50
      # otherwise label as discordant
      pseudo_ic50_concordant = if (
    ...
    list(
      pseudo_ic50 = min(df$`Compound Concentration (nM)`),
      pseudo_ic50_relation= "<",
      pseudo_ic50_concordant = "concordant"
    )
  } else if (!any(df$below_50)) {
    list(
      pseudo_ic50 = max(df$`Compound Concentration (nM)`),
      pseudo_ic50_relation= ">",
      pseudo_ic50_concordant = "concordant"
    )
  } else {
    list(
      ...
      pseudo_ic50_relation = NA_character_,
      pseudo_ic50_concordant = NA_character_
    )
  }
}
...
library(chemCal)
pseudo_ic50_raw <- single_dose_classification %>%
  mutate(
    pseudo_ic50 = map(
      data,
      compute_pseudo_ic50
...
pseudo_ic50 <- fread("classification/okl_eurofins_pseudo_ic50.csv")

...
write_csv(
  single_dose_classification %>%
    select(where(negate(is.list))),
  "classification/dose_response_classification.csv"
)
single_dose_classification <- read_csv("classification/dose_response_classification.csv")
...
write_csv(
  pseudo_ic50,
  "classification/pseudo_ic50.csv"
)
...
p <- pseudo_ic50_comp %>%
  ...
  ggplot(
    aes(classification, value, color = classification)
  ) +
  geom_quasirandom() +
  geom_boxplot(fill = NA) +
  scale_y_log10() +
  labs(y = "IC50 ChEMBL", x = "Classification Eurofins")

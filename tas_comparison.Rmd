...
kds_tas <- kinomescan_kd_fits %>%
    ...
    mutate(
        across(tas_kd, \(x) factor(as.character(x), levels = c("1", "2", "3", "10")))
    ) %>%
    # semi_join(
    #     compound_stats %>%
...
p <- tas_heatmap(kds_tas_clustered, lspci_id, hgnc_symbol, tas_kd) +
    labs(x = "Kinase", y = "Compound", fill = "TAS") +
    coord_equal() +
    theme_bw() +
    theme(
...
kds_tas_clustered_2 <- kds_tas %>%
    mutate(across(tas_kd, \(x) as.numeric(as.character(x)))) %>%
    cluster_df(lspci_id, hgnc_symbol, tas_kd) %>%
    mutate(across(tas_kd, \(x) factor(as.character(x), levels = c("1", "2", "3", "10"))))
...
tas_reduced_clustered_2 <- tas_reduced_clustered %>%
    filter(
        lspci_id %in% kds_tas_clustered_2$lspci_id,
        symbol %in% kds_tas_clustered_2$hgnc_symbol
    ) %>%
    mutate(
        lspci_id = factor(lspci_id, levels = levels(kds_tas_clustered_2$lspci_id)),
        symbol = factor(symbol, levels = levels(kds_tas_clustered_2$hgnc_symbol))
    ) %>%
    group_by(lspci_id, symbol) %>%
    arrange(tas) %>%
    slice_head(n = 1) %>%
...
ggsave(
    "plots/tas_heatmap_pre_okl_vs_okl_fitted_alternate_colors.pdf",
    p, width = 10, height = 4
)
...
pwalk(
    ...
    \(dataset, p_combined, p_combined2, ...) {
        ggsave(
            paste0("plots/tas_heatmap_pre_okl_vs_okl_", dataset, "_margins_fitted.pdf"),
...
tas_clustered_both_alluvial <- tas_clustered_both_2 %>%
    ...
    mutate(
        ...
        added_tas = if_else(added == "added", `OKL`, NA_character_)
    )
...
p <- tas_clustered_both_alluvial_long %>%
    ...
    scale_fill_manual(values = tas_colors, guide = "none", na.value = "gray50") +
...
*with new fitted kd values*
# A tibble: 4 × 5
  OKL   `Pre-OKL`  freq added added_tas
  <fct> <fct>     <int> <fct> <chr>
1 10    no data   24719 added 10
2 3     no data    5466 added 3
3 2     no data    3426 added 2
4 1     no data    1167 added 1
...
# A tibble: 20 × 5
 ...
 1 10      no data   24719 added    10
 2 3       no data    5466 added    3
 3 10      3          4739 existing NA
 4 3       10         4724 existing NA
 5 2       no data    3426 added    2
 6 2       10         1568 existing NA
 7 2       3          1461 existing NA
 8 1       no data    1167 added    1
 9 1       2           741 existing NA
10 3       2           601 existing NA
11 2       1           448 existing NA
12 no data 10          418 existing NA
13 1       3           304 existing NA
14 1       10          268 existing NA
15 10      2           216 existing NA
16 no data 3            85 existing NA
17 3       1            66 existing NA
18 10      1            55 existing NA
19 no data 2            45 existing NA
20 no data 1            25 existing NA

# A tibble: 12 × 9
   `Pre-OKL` OKL    freq added    added_tas total_freq_pre_okl total_freq_okl prop_pre_okl prop_okl
   <fct>     <fct> <int> <fct>    <chr>                  <int>          <int>        <dbl>    <dbl>
 1 3         10     4739 existing NA                      9365          62514      0.506   0.0758
 2 10        3      4724 existing NA                     39763          13633      0.119   0.347
 3 10        2      1568 existing NA                     39763           8694      0.0394  0.180
 4 3         2      1461 existing NA                      9365           8694      0.156   0.168
 5 2         1       741 existing NA                      3394           4322      0.218   0.171
 6 2         3       601 existing NA                      3394          13633      0.177   0.0441
 7 1         2       448 existing NA                      2436           8694      0.184   0.0515
 8 3         1       304 existing NA                      9365           4322      0.0325  0.0703
 9 10        1       268 existing NA                     39763           4322      0.00674 0.0620
10 2         10      216 existing NA                      3394          62514      0.0636  0.00346
11 1         3        66 existing NA                      2436          13633      0.0271  0.00484
12 1         10       55 existing NA                      2436          62514      0.0226  0.000880

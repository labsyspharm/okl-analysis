...
kd_fit_pairs <- synGlob("syn73756839", "*.csv.gz") |>
  set_names(\(x) str_remove(basename(x), fixed(".csv.gz"))) |>
  map(syn) |>
  map(read_csv) |>
  ...
  mutate(
    ...
    across(
      summary_json,
      \(x) map(x, \(y) as_tibble(fromJSON(y)$data))
    )
  )

kd_fit_pairs_summaries <- kd_fit_pairs %>%
  select(combination, compound_id, target, status, summary_json) %>%
  unnest(summary_json)
...
```{r}
kd_fit_metrics <- bind_rows(
  kd_fits_summaries |>
    mutate(combination = "all"),
  kd_fit_pairs_summaries
) %>%
  filter(index %in% c("kd_log", "hill_slope")) %>%
  pivot_wider(
    names_from = index,
    values_from = -c(compound_id, target, index, combination)
  ) %>%
  transmute(
    combination, compound_id, target,
...
write_csv(
  kd_table |>
    filter(combination != "all"),
  "classification/okl_eurofins_kd_fits_pairs.csv.gz"
)
```

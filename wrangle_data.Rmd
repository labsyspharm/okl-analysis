...
```{r}
single_dose_processed <- bind_rows(
  single_dose_lspci_id_no_repeat %>%
  ...
  select(
    ...
    experiment_group, experiment, everything()
  )

fwrite(single_dose_processed, here("data", "okl_single_dose.csv.gz"))
```
...
single_dose_datasets <- bind_rows(
  only_original = single_dose_processed %>%
    filter(experiment_group == "original"),
  original_repeat_replaced = single_dose_processed %>%
    filter(experiment_group == "original") %>%
    anti_join(
      single_dose_processed %>%

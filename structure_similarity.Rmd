...
mat_compound_info <- okl_compound_info %>%
  ...
  power_inner_join(
    select(clust_cut, lspci_id, cluster_tanimoto = cluster),
    by = "lspci_id",
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_left = "warn",
  ...
  power_inner_join(
    select(clust_cut_kd, lspci_id, cluster_kd = cluster),
    by = "lspci_id",
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_left = "warn",
  ...
  mutate(
    across(starts_with("cluster"), \(x) fct_inseq(as.character(x)))
  )
...
row_anno <- createAnnotation(
  ...
  which = "row",
  colors = list(
    cluster_kd = \(x) ggokabeito::palette_okabe_ito(seq_len(x)),
    cluster_tanimoto = \(x) ggokabeito::palette_okabe_ito(seq_len(x))
  ),
  ...
  annotation_width = unit(c(30, 2, 2), "mm"),
  gap = unit(0, "mm")
)
...
dend_kd <- as.dendrogram(clust_kd) %>%
  branches_attr_by_clusters(
    clusters = clust_cut_kd$cluster[clust_kd$order],
    values = cluster_col_map[unique(clust_cut_kd$cluster[clust_kd$order])]
  )
```

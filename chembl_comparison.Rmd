...
ggsave(
    "plots/chembl_kd_vs_kinomescan_pseudo_kd_new2.pdf", p_scatter,
    width = 8, height = 4
)
...
ggsave(
    "plots/chembl_kd_by_comparison_combined_new2.pdf", p_combined,
    width = 6, height = 4.5
)
```
...
p <- single_dose_comp %>%
  mutate(
    ...
    chembl_kd = pmin(chembl_kd, 30000)
  ) %>%
...
single_dose_comp_scores_weighted_optimal <- single_dose_comp_scores_weighted %>%
  select(-roc, -roc_measures) %>%
  mutate(
    across(
      scores,
      \(x) map(x, \(y) slice_max(y, score))
    )
  ) %>%
  unnest(scores) %>%
  mutate(Cutoff = 100 - Cutoff)
...
optimal_params_table <- single_dose_comp_scores_weighted_diagnostics %>%
  inner_join(

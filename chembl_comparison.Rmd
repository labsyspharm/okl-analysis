...
qsave(
  ...
  file.path("classification", "okl_eurofins_single_dose_chembl_kd_comparison_roc.qs")
)
write_csv(
  single_dose_comp_binned_filtered,
  file.path("classification", "okl_eurofins_single_dose_chembl_kd_comparison_roc.csv.gz")
)

synStoreMany(
  c(
    "classification/okl_eurofins_single_dose_chembl_kd_comparison_roc.qs",
    "classification/okl_eurofins_single_dose_chembl_kd_comparison_roc.csv.gz"
  ),
...
single_dose_comp_rocs <- single_dose_comp_binned_filtered %>%
  ...
  mutate(
    ...
    roc_measures = map(
      roc,
      ~measureit(.x, measure = c("SENS", "SPEC", "PREC", "PPV")) %>%
        unclass() %>%
        as_tibble()
    )
  )

single_dose_comp_aucs <- single_dose_comp_rocs %>%
  mutate(
    auc = map_dbl(roc, "AUC")
  )

single_dose_comp_best_doses <- single_dose_comp_aucs %>%
...
single_dose_comp_roc_values <- single_dose_comp_rocs %>%
  transmute(
    `Compound Concentration (nM)`,
    kd_threshold = fct_inorder(
      paste(kd_threshold, "nM")
    ),
    roc_values = map(roc, plot)
  )

fn_fp_weights <- c("2xFN" = 2/3, "equal" = .5, "2xFP" = 1/3)
compute_weighted_score <- function(df, fn_fp_weight) {
  df %>%
    mutate(
      score = 2 * (fn_fp_weight * (TP / (TP + FN)) + (1 - fn_fp_weight) * (TN / (TN + FP))) - 1,
      score_naive = fn_fp_weight * FN + (1 - fn_fp_weight) * FP
    )
}

single_dose_comp_scores_weighted <- single_dose_comp_rocs %>%
  cross_join(
    enframe(
      fn_fp_weights,
      name = "weight_name",
  ...
  mutate(
    scores = map2(roc_measures, fn_fp_weight, compute_weighted_score)
  ) %>%
  select(-roc, -roc_measures)


single_dose_comp_scores_weighted_diagnostics <- single_dose_comp_scores_weighted %>%
  unnest(scores) %>%
  pivot_longer(
    c(TP, TN, FP, FN, score, score_naive, SENS, SPEC, PREC, PPV),
    names_to = "variable",
    values_to = "value"
  ) %>%
  filter(
    case_when(
      variable %in% c("score", "score_naive") ~ TRUE,
      TRUE ~ weight_name == "equal"
    )
  ) %>%
  mutate(
    Cutoff = 100 - Cutoff
...
single_dose_comp_scores_weighted_diagnostic_optim <- single_dose_comp_scores_weighted_diagnostics %>%
  filter(str_starts(variable, "score")) %>%
  group_nest(
    variable, `Compound Concentration (nM)`, kd_threshold, weight_name
  ) %>%
  mutate(
    data = map2(
      data, variable,
...
single_dose_comp_scores_weighted_diagnostic_optim_and_flat <- single_dose_comp_scores_weighted_diagnostics %>%
  filter(str_starts(variable, "score")) %>%
  group_nest(
    variable, `Compound Concentration (nM)`, kd_threshold, weight_name
  ) %>%
  crossing(optim = c("flat", "optim")) %>%
  mutate(
    data = pmap(
      .,
      \(data, variable, optim, ...) {
        if (optim == "optim") {
          if (variable == "score")
            slice_max(data, value)
          else
            slice_min(data, value)
        } else
          slice(data, which.min(abs(Cutoff - 35)))
      }
    )
  ) %>%
  unnest(data)

measure_names <- tribble(
  ~abbreviation, ~name, ~name_long,
  "SENS", "Sensitivity", "Sensitivity TP / (TP + FN)",
  "SPEC", "Specificity", "Specificity TN / (TN + FP)",
  "PREC", "Precision", "Precision TP / (TP + FP)",
  "score", "Weighted Youden", "Weighted Youden",
  "score_naive", "Weighted Score", "Weighted Score",
  "TP", "True Positives", "True Positives TP",
  "TN", "True Negatives", "True Negatives TN",
  "FP", "False Positives", "False Positives FP",
  "FN", "False Negatives", "False Negatives FN"
)

library(ggrepel)
p <- single_dose_comp_scores_weighted_diagnostics %>%
  filter(
    variable %in% c("score", "SENS", "SPEC", "PREC")
  ) %>%
  left_join(
    measure_names,
    by = c("variable" = "abbreviation")
  ) %>%
  ggplot(
    aes(
      Cutoff, value, color = name_long,
      linetype = weight_name
    )
  ) +
  geom_line() +
  geom_point(
    data = single_dose_comp_scores_weighted_diagnostic_optim %>%
      filter(variable %in% c("score")) %>%
      left_join(
        measure_names,
        by = c("variable" = "abbreviation")
      ),
  ...
  facet_grid(vars(kd_threshold), vars(`Compound Concentration (nM)`)) +
  labs(
    x = "Cutoff", y = "Value",
    color = "Variable", linetype = "Weights"
  )

ggsave(
  file.path("plots", "kd_roc_weighted_scores.pdf"),
  width = 8, height = 5
)

p <- single_dose_comp_scores_weighted_diagnostics %>%
  filter(
    variable %in% c("score_naive", "FP", "FN")
  ) %>%
  left_join(
    measure_names,
    by = c("variable" = "abbreviation")
  ...
  geom_point(
    data = single_dose_comp_scores_weighted_diagnostic_optim %>%
      filter(variable %in% c("score_naive")) %>%
      left_join(
        measure_names,
        by = c("variable" = "abbreviation")
      ),
...
ggsave(
  file.path("plots", "kd_roc_weighted_score_naive.pdf"),
  width = 8, height = 5
)
...
optimal_params_table_raw <- single_dose_comp_scores_weighted_diagnostics %>%
  inner_join(
    single_dose_comp_scores_weighted_diagnostic_optim %>%
      filter(variable == "score") %>%
  ...
  bind_rows(
    single_dose_comp_scores_weighted_diagnostic_optim %>%
      ...
      transmute(
        `Compound Concentration (nM)`, kd_threshold,
        weight_name,
        value = Cutoff, variable = "Cutoff"
      ) %>%
      distinct()
  ) %>%
  filter(
    weight_name == "equal",
    variable %in% c("Cutoff", "FP", "FN", "SENS", "SPEC", "PREC")
  ) %>%
  select(
    `Compound Concentration (nM)`, kd_threshold,
    variable, value
...
optimal_params_gt <- optimal_params_table %>%
  ...
  data_color(
    columns = everything(),
    rows = variable %in% c("SENS", "SPEC", "PREC"),
    direction = "row",
    fn = \(x) {
      # scales::col_numeric(
      #   palette = c("white", "red"),
...
param_comparison_table <- single_dose_comp_scores_weighted_diagnostic_optim %>%
  filter(variable == "score", weight_name == "equal") %>%
  select(-variable, -value, -Depth, -weight_name, -fn_fp_weight) %>%
  mutate(optim = "optim") %>%
  bind_rows(
    select(., -Cutoff) %>%
    distinct() %>%
    crossing(
      Cutoff = c(1, 10, 20, 35, 50, 60, 70)
    ) %>%
    mutate(optim = "flat")
  ) %>%
  # Make sure Cutoffs are not present twice (once in optim and once in flat)
  group_by(`Compound Concentration (nM)`, kd_threshold, Cutoff) %>%
  filter(
    if (uniqueN(optim) == 1 || cur_group()$Cutoff == 35)
      TRUE
    else
      optim == "optim"
  ) %>%
  ...
  mutate(
    hit_kinomescan = `Percent Control` <= Cutoff,
...
param_comparison_table_optim_vs_flat <- param_comparison_table %>%
  select(
    `Compound Concentration (nM)`, kd_threshold, optim, Cutoff,
    lspci_id, hmsl_id, name, library, `DiscoveRx Gene Symbol`, `Entrez Gene Symbol`,
    starts_with("hit")
  ) %>%
  filter(optim == "optim" | Cutoff == 35) %>%
  # select(-Cutoff) %>%
  pivot_wider(
    names_from = optim,
    values_from = c(Cutoff, starts_with("hit"))
  ) %>%
  mutate(
...
param_comparison_table_optim_vs_flat_best_doses_coral %>%
  filter(name == "RUXOLITINIB", kd_threshold == 1000) %>%
  transmute(
    CORAL, size = if_else(change_class == "same", 1, 100)
  ) %>%
  clipr::write_clip()
...
rocs_per_compound <- single_dose_comp_binned_filtered %>%
  crossing(
    kd_threshold = KD_THRESHOLDS
  ) %>%
  group_by(lspci_id, hmsl_id, name, `Compound Concentration (nM)`, kd_threshold) %>%
  summarize(
    roc = possibly(rocit)(
      100 - `Percent Control`,
      # `Percent Control`,
  ...
  mutate(
    ...
    roc_measures = map(
      roc,
      ~possibly(measureit)(.x, measure = c("SENS", "SPEC", "PREC", "PPV")) %>%
        unclass() %>%
        as_tibble()
    ),
    roc_values = map(
...
single_dose_comp_pr_plots <- single_dose_comp_rocs %>%
  ...
  facet_wrap(~kd_threshold) +
  coord_cartesian(xlim = c(0, 1.1))

ggsave(
  "plots/percent_control_kd_pr.pdf",
...
single_dose_comp_roc_plot <- single_dose_comp_roc_values %>%
  ...
  labs(
    ...
    color = "Assay concentration",
    title = "ROC curve for predicting an Kd below threshold"
  ) +

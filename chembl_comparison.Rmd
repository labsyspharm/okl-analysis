...
Make scatterplot of ChEMBL IC50 vs KINOMEscan pseudo-IC50
...
chembl_kd <- chembl_kd_raw %>%
  ...
  inner_join(
    compound_mapping %>%
      select(lspci_id, chembl_id_compound = external_id),
...
pseudo_ic50_kd_comp <- pseudo_ic50_class %>%
  group_by(
    lspci_id, hmsl_id, name, `Entrez Gene Symbol`, lspci_target_id
  ) %>%
  arrange(classification == "discordant", pseudo_ic50_relation != "=", pseudo_ic50) %>%
  slice_head(n = 1) %>%
  ungroup() %>%
...
p_scatter <- pseudo_ic50_kd_comp %>%
  ...
  ggplot(
    # aes(value, pseudo_ic50, color = classification, shape = pseudo_ic50_relation)
    aes(chembl_kd, pseudo_ic50, color = classification)
  ) +
  scale_color_manual(values = class_colors) +
  geom_point(alpha = 0.5, shape = 16, size = 1) +
...
ggsave(
    "plots/chembl_kd_vs_kinomescan_pseudo_ic50.pdf", p_scatter,
    width = 8, height = 4
)

p_beeswarm <- pseudo_ic50_kd_comp %>%
  filter(
    pseudo_ic50_concordant == "concordant",
    # pseudo_ic50_relation == "=",
    classification != "discordant"
...
ggsave(
    "plots/chembl_kd_by_comparison_combined.pdf", p_combined,
    width = 6, height = 4.5
)
```
...
pseudo_ic50_comp_both <- bind_rows(
  ic50 = pseudo_ic50_comp %>%
    filter(
      pseudo_ic50_concordant == "concordant",
      # pseudo_ic50_relation == "=",
      classification != "discordant"
    ) %>%
    select(lspci_id, `DiscoveRx Gene Symbol`, gene_symbol = hgnc_symbol, value, pseudo_ic50, classification),
  kd = pseudo_ic50_kd_comp %>%
    filter(
      pseudo_ic50_concordant == "concordant",
      # pseudo_ic50_relation == "=",
      classification != "discordant"
    ) %>%
    select(lspci_id, `DiscoveRx Gene Symbol`, gene_symbol, value = chembl_kd, pseudo_ic50, classification),
  .id = "chembl_type"
)

pseudo_ic50_comp_cor_res <- pseudo_ic50_comp_both %>%
  group_by(chembl_type) %>%
  summarize(
    res = cor.test(
      ~value + pseudo_ic50, data = cur_data()
    ) %>%
      broom::tidy() %>%
      list(),
    n = n(),
...
pseudo_ic50_comp_false_class_by_compound <- pseudo_ic50_comp_false_class %>%
  count(chembl_type, lspci_id, false_class) %>%
  group_by(chembl_type, lspci_id) %>%
  mutate(
    prop = n / sum(n)
  )

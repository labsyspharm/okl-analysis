...
Same thing for new fitted Kd values
...
p_scatter <- pseudo_kd_comp %>%
  ...
  ggplot(
    ...
    aes(
      ...
      text = paste(
        name, `DiscoveRx Gene Symbol`,
        "<br>ChEMBL", signif(value, 2), "nM Eurofins", signif(kd, 2), "nM"
      )
    )
  ) +
  geom_point(aes(size = fda_target, shape = kd_relation)) +
  geom_text(
    aes(label = label),
    data = \(df) df %>%
      mutate(across(c(value, kd), \(x) log10(x))) %>%
      summarize(
        res = cor.test(
          value, kd
        ) %>%
      ...
      unnest(res) %>%
      mutate(label = paste("R ==", signif(estimate, 2))),
    x = -Inf, y = Inf, hjust = 0, vjust = 1,
    inherit.aes = FALSE, parse = TRUE
  ) +
  scale_shape_manual(
...
ggsave(
    "plots/chembl_ic50_vs_kinomescan_fit_kd_new.pdf", p_scatter,
    width = 8, height = 5
)
...
p_scatter <- pseudo_kd_kd_comp %>%
  ...
  mutate(
    across(c(kd, chembl_kd), \(x) pmin(x, 10000)),
    kd_relation = if_else(kd < .1, "<", "="),
    across(kd, \(x) pmax(x, .1)),
    tas_kd = as.character(tas_kd) %>%
      factor(levels = names(tas_colors))
  ) %>%
  arrange(desc(fda_target)) %>%
  ggplot(
    aes(chembl_kd, kd, fill = tas_kd)
  ) +
  geom_point(
    aes(size = fda_target, shape = kd_relation, color = fda_target),
  ) +
  geom_text(
    aes(label = label),
    data = \(df) df %>%
      mutate(across(c(chembl_kd, kd), \(x) -log10(x))) %>%
      summarize(
        res = cor.test(
          chembl_kd, kd
        ) %>%
...
p_beeswarm <- pseudo_kd_kd_comp %>%
  ...
  ggplot(
    aes(chembl_kd, tas_kd, color = tas_kd)
  ) +
  geom_quasirandom(orientation = "y", shape = 16, size = 1, alpha = 0.5, width = 0.3) +
  geom_boxplot(fill = NA, outlier.shape = NA) +
...
ggsave(
    "plots/chembl_kd_vs_kd_fit_comparison_combined_tas_with_fda.pdf", p_combined,
    width = 6, height = 4.5
)
...
pseudo_kd_kd_comp |>
  summarize(
    lm_res = list(
      lm(log10(kd) ~ log10(chembl_kd), data = cur_data()) %>%
        broom::tidy()
    )
  ) |>
  unnest(lm_res)
...
pseudo_kd_kd_comp %>%
  ...
  # geom_abline(
  #   slope = .626, intercept = 2612,
  #   color = "black"
  # ) +
  geom_abline(
    slope = .875, intercept = 10^2.74,
    color = "black"
  ) +
  geom_text(
    aes(label = label),
...
param_comparison_table_optim_vs_flat_best_doses_coral %>%
  filter(name == "RUXOLITINIB", kd_threshold == 1000) %>%
  count(
    hit_class_optim, hit_class_flat
  )
# # A tibble: 5 × 3
#   hit_class_optim hit_class_flat     n
#   <chr>           <chr>          <int>
# 1 FN              FN                 2
# 2 FP              FP                64
# 3 TN              FP                30
...
single_dose_comp_proc_scores_best_and_series_and_all <- single_dose_comp_proc_scores %>%
  ...
  mutate(
    ...
    threshold_label = case_when(
      point_type == "best" ~ paste0("*", signif(threshold, 2), "%"),
      point_type == "flat" ~ paste0(as.character(signif(threshold, 2)), "%"),
      TRUE ~ ""
    )
  ) %>%
  left_join(
    filter(., point_type %in% c("best", "flat")) %>%
      group_by(`Compound Concentration (nM)`, kd_threshold) %>%
...
ggsave(
  "plots/percent_control_kd_pr_flat.pdf",
...
ggsave(
  "plots/percent_control_kd_roc_flat.pdf",
  single_dose_comp_roc_plots,
  width = 16, height = 6
)
```

Alternative version with symbols for quartiles
...
single_dose_comp_roc_plots <- single_dose_comp_proc_scores_best_and_series_and_all %>%
  ...
  mutate(
    point_label = case_when(
      point_type == "series" ~ paste0(threshold_label, "%"),
      point_type == "best" ~ "optimal",
      TRUE ~ ""
    )
  ) %>%
  ggplot(
  ...
  geom_text_repel(
    aes(
      ...
      # fontface = if_else(point_type == "best", "bold", "plain")
    ),
    # data = \(x) mutate(
    #   x,
    #   threshold_label = if_else(
    ...
    data = \(x) filter(
      ...
      mutate(
        threshold_label = str_replace(threshold_label, fixed("*"), "%")
      ),
    box.padding = 0.5,
    show.legend = FALSE,
    max.iter = 50000,
...
ggsave(
  "plots/percent_control_kd_roc_new_symbols.pdf",
  single_dose_comp_roc_plots,

---
title: "Analysis of OKL selectivity data"
output: html_document
editor_options:
  chunk_output_type: console
---

```{r setup}
library(tidyverse)
library(synExtra)
library(here)
library(fst)
library(data.table)
library(powerjoin)
# library(dtplyr)

theme_set(theme_minimal())

synapser::synLogin()
syn <- synDownloader("~/data/", .cache = TRUE)
```

```{r loading}
inputs <- list(
  okl_single_dose_datasets = "syn52504516",
  kinase_info = "syn51286743",
  klaeger_ic50s = "syn51288931",
  nominal_targets = "syn52947779",
  okl_kds = "syn69680641",
  okl_classification = "syn51080579"
)

input_files <- map(
  inputs, syn
)

single_dose_datasets <- input_files[["okl_single_dose_datasets"]] %>%
  fread() %>%
  as_tibble()

kinase_info <- input_files[["kinase_info"]] %>%
  read_csv()

klaeger_ic50s <- input_files[["klaeger_ic50s"]] %>%
  fread() %>%
  as_tibble()

nominal_targets <- input_files[["nominal_targets"]] %>%
  fread() %>%
  as_tibble() %>%
  power_inner_join(
    single_dose_datasets %>%
      distinct(lspci_id, hmsl_id),
    by = c("HMSLID" = "hmsl_id"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn"
    )
  )

kds <- input_files[["okl_kds"]] %>%
  fread() %>%
  as_tibble()

classification <- input_files[["okl_classification"]] %>%
  fread() %>%
  as_tibble()

coral_families <- syn("syn61680422") %>%
  read_csv()

chemical_probes_raw <- syn("syn74301118") |>
  read_csv()

compound_mapping <- syn("syn26260389") %>%
  read_fst(as.data.table = TRUE)
```


## Process Chemical probes

```{r}
chemical_probes_mapped <- chemical_probes_raw |>
  power_inner_join(
    compound_mapping %>%
      filter(source == "chembl") |>
      distinct(lspci_id, chembl_id = external_id),
    by = c("ChEMBL ID" = "chembl_id"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn"
    )
  )
```

## CATDS

### Using Single dose data to compute CATDS

For computing CATDS try to reduce noise by censoring Percent Control values of
compound-target pairs that are classified as non-binding. We frequently have
values of 90% activity remaining for non-binding compounds, which is likely
just noise but affects CATDS calculation significantly.

Setting Percent Control to 100% for measurements classified as non-binding
AND where Percent Control >35%

```{r}
catds <- single_dose_datasets %>%
  filter(
    `Compound Concentration (nM)` %in% c(12.5, 100, 1000, 10000),
    !exclude_target,
    !qpcr_miss
  ) %>%
  power_left_join(
    select(classification, dataset, lspci_id, `DiscoveRx Gene Symbol`, classification),
    by = c("dataset", "lspci_id", "DiscoveRx Gene Symbol"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn"
    )
  ) %>%
  mutate(
    non_binding_censored = FALSE
  ) %>%
  bind_rows(
    mutate(
      .,
      non_binding_censored = TRUE,
      `Percent Control` = if_else(
        classification == "non-binding" & `Percent Control` > 35,
        100, `Percent Control`
      )
    )
  ) %>%
  group_by(non_binding_censored, dataset, lspci_id, `Compound Concentration (nM)`) %>%
  filter(n() > 50) %>%
  mutate(
    catds = (100 - `Percent Control`) / sum(100 - `Percent Control`)
  ) %>%
  ungroup() %>%
  select(dataset, non_binding_censored, everything())

catds_max <- catds %>%
  group_by(dataset, non_binding_censored, lspci_id, `Compound Concentration (nM)`) %>%
  slice_max(
    catds,
    n = 1,
    na_rm = TRUE,
    with_ties = TRUE
  ) %>%
  ungroup()

catds %>%
  group_nest(non_binding_censored) %>%
  pwalk(
    \(non_binding_censored, data) {
      write_csv(
        data %>%
          select(where(negate(is.list))),
        paste0("classification/okl_eurofins_catds", if (non_binding_censored) "_censored" else "", ".csv")
      )
    }
  )

catds_max %>%
  group_nest(non_binding_censored) %>%
  pwalk(
    \(non_binding_censored, data) {
      write_csv(
        data %>%
          select(where(negate(is.list))),
        paste0("classification/okl_eurofins_catds_index_max", if (non_binding_censored) "_censored" else "", ".csv")
      )
    }
  )

```

CATDSmax here can include ties. This happens especially when a compound has
multiple targets with the same potency at the limit of our resolution (e.g. <12.5nM)


```{r}
catds %>%
  filter(
    `Compound Concentration (nM)` %in% c(12.5, 100, 1000, 10000)
  ) %>%
  ggplot(
    aes(catds, color = fct_inseq(as.character(`Compound Concentration (nM)`)))
  ) +
  geom_density() +
  facet_wrap(~dataset) +
  scale_y_log10()

catds_max %>%
  filter(
    `Compound Concentration (nM)` %in% c(12.5, 100, 1000, 10000)
  ) %>%
  ggplot(
    aes(catds, color = fct_inseq(as.character(`Compound Concentration (nM)`)))
  ) +
  geom_density() +
  facet_wrap(~dataset + non_binding_censored)

catds_max %>%
  ggplot(
    aes(catds, color = fct_inseq(as.character(`Compound Concentration (nM)`)))
  ) +
  stat_ecdf() +
  facet_wrap(~dataset + non_binding_censored) +
  lims(x = c(0, 1))

catds %>%
  filter(name == "TOFACITINIB", str_detect(hgnc_symbol, "JAK")) %>%
  View()

catds %>%
  mutate(target_engagement = 100 - `Percent Control`) %>%
  filter(name == "TOFACITINIB", `Compound Concentration (nM)` == 12.5) %>%
  View()
```

### Using Kd data to compute CATDS

```{r}
kds_filtered <- kds %>%
  mutate(
    across(kd, \(x) pmin(x, 10000))
  ) %>%
  filter(
    !exclude_target,
    classification != "discordant"
  ) |>
  power_inner_join(
    coral_families %>%
      select(`DiscoveRx Gene Symbol`, Family),
    by = c("DiscoveRx Gene Symbol"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn"
    )
  )

catds_most_potent <- kds_filtered |>
  group_by(lspci_id, dataset) %>%
  slice_min(kd, with_ties = FALSE) %>%
  ungroup() %>%
  inner_join(
    kds_filtered |>
      group_nest(dataset, lspci_id),
    by = c("dataset", "lspci_id")
  ) %>%
  mutate(
    catds_data = map2(
      data, kd,
      \(kd_data, kd_most_potent) {
        kd_data |>
          mutate(
            kd_most_potent = kd_most_potent,
            percent_remaining_catds = 100 - 100 / (1 + (kd / kd_most_potent)**hill_slope),
            catds = (100 - percent_remaining_catds) / sum(100 - percent_remaining_catds, na.rm = TRUE)
          )
      }
    )
  ) |>
  select(
    dataset, lspci_id, catds_data
  ) |>
  unnest(catds_data)


write_csv(
  catds_most_potent %>%
    arrange(
      desc(catds)
    ),
  "classification/okl_eurofins_catds_kd_most_potent.csv.gz"
)
```


```{r}
catds_most_potent_by_gene <- catds_most_potent |>
  group_by(
    dataset, lspci_id, hmsl_id, library, name,
    hgnc_symbol, entrezgene_id, ensembl_gene_id, `Entrez Gene Symbol`
  ) |>
  summarize(
    catds = sum(catds, na.rm = TRUE),
    kd_most_potent = min(kd, na.rm = TRUE),
    kd_list = paste(paste0(`DiscoveRx Gene Symbol`, ": ", signif(kd, 2)), collapse = "; "),
    .groups = "drop"
  )
```

* Plot dose response curves for potential chemical probes
* Share table with Caitlin
* Use only wild type kinases
* Compute family-wise chemical probe

#### By family


```{r}

catds_most_potent_by_fam <- catds_most_potent |>
  power_inner_join(
    coral_families %>%
      select(`DiscoveRx Gene Symbol`, Family),
    by = c("DiscoveRx Gene Symbol"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn"
    )
  ) |>
  group_by(dataset, lspci_id, hmsl_id, library, name, Family) |>
  summarize(
    catds = sum(catds, na.rm = TRUE),
    .groups = "drop"
  )

```

### Chemical probes

Check if any of our compounds fits the criteria for chemical probes. Potency <100nM
and CATDSmax >.5

```{r}
chemical_probes_catds <- catds_most_potent |>
  filter(catds > .5, kd_most_potent < 100, library == "OKL") |>
  arrange(desc(catds))

write_csv(
  chemical_probes_catds %>%
    select(where(negate(is.list))),
  "classification/okl_eurofins_chemical_probes_single_target_catds.csv"
)

chemical_probes_by_gene_catds <- catds_most_potent_by_gene |>
  filter(catds > .5, kd_most_potent < 100, library == "OKL") |>
  arrange(desc(catds))

write_csv(
  chemical_probes_by_gene_catds,
  "classification/okl_eurofins_chemical_probes_by_gene_catds.csv"
)

chemical_probes <- catds_max %>%
  power_inner_join(
    pseudo_kd %>%
      select(dataset, lspci_id, `DiscoveRx Gene Symbol`, pseudo_kd),
    by = c("dataset", "lspci_id", "DiscoveRx Gene Symbol"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn"
    )
  ) %>%
  filter(
    catds > 0.5,
    pseudo_kd < 100
  )


write_csv(
  chemical_probes %>%
    select(where(negate(is.list))),
  "classification/okl_eurofins_chemical_probes.csv"
)

chemical_probe_kd <- pseudo_kd %>%
  filter(dataset == "original_repeat_replaced") %>%
  semi_join(
    chemical_probes,
    by = c("lspci_id")
  ) %>%
  arrange(pseudo_kd) %>%
  mutate(
    across(`DiscoveRx Gene Symbol`, fct_inorder)
  )

p <- chemical_probe_kd %>%
  ggplot(aes(`DiscoveRx Gene Symbol`, pseudo_kd)) +
    geom_step() +
    geom_point() +
    ggrepel::geom_text_repel(
      aes(label = paste(`DiscoveRx Gene Symbol`, signif(pseudo_kd, 2))),
      data = \(x) filter(x, pseudo_kd < 1000),
      max.overlaps = Inf
    ) +
    scale_y_log10() +
    theme_minimal() +
    theme(axis.text.x = element_blank(), panel.grid.major.x = element_blank())

ggsave(
  file.path("plots", "chemical_probes_kd_tak_333.pdf"),
  p, width = 7, height = 4
)

catds %>%
  filter(dataset == "original_repeat_replaced") %>%
  semi_join(
    chemical_probes,
    by = c("lspci_id")
  ) %>%
  arrange(catds) %>%
  View()

catds %>%
  count(dataset, `DiscoveRx Gene Symbol`, lspci_id, `Compound Concentration (nM)`) %>%
  count(n)
```

## Partition index

Compute partition index

Analysis of kinase inhibitor selectivity using a thermodynamics-based partition index
Alan C Cheng  1 , John Eksterowicz, Stephanie Geuns-Meyer, Yaxiong Sun


```{r}
kds_thresholded <- kds %>%
  mutate(
    kd = pmin(kd, 10000)
  )


partition_index <- kds_thresholded %>%
  # filter(pseudo_ic50_relation == "=") %>%
  filter(
    !exclude_target,
    classification != "discordant"
  ) %>%
  group_by(dataset, lspci_id) %>%
  filter(n() > 50) %>%
  mutate(
    partition_index = (1 / kd) / sum(1 / kd, na.rm = TRUE)
  ) %>%
  ungroup()

partition_index_max <- partition_index %>%
  group_by(
    dataset, lspci_id, hmsl_id, library, name
  ) %>%
  slice_max(
    partition_index,
    n = 1,
    na_rm = TRUE,
    with_ties = TRUE
  ) %>%
  ungroup()

write_csv(
  partition_index %>%
    select(where(negate(is.list))),
  "classification/okl_eurofins_partition_index.csv"
)

write_csv(
  partition_index_max %>%
    select(where(negate(is.list))),
  "classification/okl_eurofins_partition_index_max.csv"
)
```

Same as CATDSmax, partition_index_max can include ties.

```{r}
partition_index_max %>%
  ggplot(aes(partition_index)) +
    geom_histogram() +
    facet_wrap(~dataset + library)

partition_index_max %>%
  ggplot(aes(partition_index, color = dataset)) +
    stat_ecdf() +
    lims(x = c(0, 1)) +
    facet_wrap(~library)
```


```{r}
klaeger_partition_index <- klaeger_ic50s %>%
  # filter(IC50_relation == "=") %>%
  group_by(lspci_id, Compound) %>%
  mutate(
    partition_index = (1 / IC50) / sum(1 / IC50, na.rm = TRUE)
  ) %>%
  ungroup()

klaeger_partition_index_max <- klaeger_partition_index %>%
  group_by(lspci_id, Compound) %>%
  summarize(
    partition_index_max = max(partition_index, na.rm = TRUE),
    .groups = "drop"
  )
```

### By Family

```{r}

partition_index_fam <- kds_filtered %>%
  power_inner_join(
    coral_families %>%
      select(`DiscoveRx Gene Symbol`, Family),
    by = c("DiscoveRx Gene Symbol"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn"
    )
  ) %>%
  # filter(pseudo_ic50_relation == "=") %>%
  group_by(dataset, lspci_id) %>%
  filter(n() > 50) %>%
  mutate(
    partition_index = (1 / kd) / sum(1 / kd, na.rm = TRUE)
  ) %>%
  ungroup()

partition_index_fam_agg <- partition_index_fam %>%
  group_by(dataset, Family, lspci_id, hmsl_id, library, name) %>%
  summarize(
    partition_index = sum(partition_index, na.rm = TRUE),
    .groups = "drop"
  )

```


## Chemical Probes with CATDs and partition index

```{r}
chemical_probe_by_family <- power_inner_join(
  catds_most_potent_by_fam,
  partition_index_fam_agg,
  by = c("dataset", "Family", "lspci_id", "hmsl_id", "library", "name"),
  check = check_specs(
    unmatched_keys_left = "warn",
    duplicate_keys_right = "warn",
    unmatched_keys_right = "warn",
    duplicate_keys_left = "warn"
  )
) |>
  power_inner_join(
    kds_filtered |>
      group_by(dataset, lspci_id, hmsl_id, library, name, Family) |>
      summarize(
        kd_most_potent_family = min(kd, na.rm = TRUE),
        kd_list = paste(paste0(`DiscoveRx Gene Symbol`, ": ", signif(kd, 2)), collapse = "; "),
        .groups = "drop"
      ),
    by = c("dataset", "Family", "lspci_id", "hmsl_id", "library", "name"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn",
      unmatched_keys_right = "warn",
      duplicate_keys_left = "warn"
    )
  )  |>
  power_inner_join(
    kds_filtered |>
      distinct(dataset, lspci_id, hmsl_id, library, name, Family) |>
      mutate(
        data_outside_family = pmap(
          cur_data(),
          \(lspci_id, Family, ...) {
            df <- kds_filtered |>
              filter(
                .data$lspci_id == .env$lspci_id,
                .data$Family != .env$Family
              )
            df |>
              arrange(kd) |>
              summarize(
                kd_most_potent_outside_family = min(kd, na.rm = TRUE),
                kd_list_top3_outside_family = paste(paste0(head(`DiscoveRx Gene Symbol`, n = 3), ": ", signif(head(kd, n = 3), 2)), collapse = "; "),
                .groups = "drop"
              )
          }
        )
      ) |>
      unnest(data_outside_family),
    by = c("dataset", "Family", "lspci_id", "hmsl_id", "library", "name"),
    check = check_specs(
      unmatched_keys_left = "warn",
      duplicate_keys_right = "warn",
      unmatched_keys_right = "warn",
      duplicate_keys_left = "warn"
    )
  )


p <- ggplot(
  chemical_probe_by_family,
  aes(
    x = catds,
    y = partition_index,
  )
) +
  geom_point()
p
```

```{r}
chemical_probe_by_family |>
  filter(partition_index > .9, catds < .5)

kds_filtered |> filter(name == "SGX-523") |> arrange(kd)
catds_most_potent |> filter(name == "SGX-523") |> arrange(desc(catds)) |> select(-c(dataset, hmsl_id, entrezgene_id))
```


```{r}
chemical_probe_by_family_top <- chemical_probe_by_family |>
  filter(
    partition_index > .5 | catds > .5,
    kd_most_potent_family < 100
  ) |>
  arrange(desc(catds))

write_csv(
  chemical_probe_by_family_top %>%
    select(where(negate(is.list))),
  "classification/okl_eurofins_chemical_probes_by_family_catds_partition_index.csv"
)
```


### Counterexamples

```{r}
chemical_probes_gt3 <- chemical_probes_mapped |>
  filter(`Rating in cell` >= 3)

chemical_probe_by_family_ce <- chemical_probe_by_family |>
  group_by(dataset, lspci_id, hmsl_id, library, name) |>
  slice_max(catds, n = 1, with_ties = FALSE) |>
  inner_join(
    chemical_probes_gt3,
    by = c("lspci_id" = "lspci_id")
  )

chemical_probe_by_family_ce |>
  arrange(catds) |>
  View()
```

```{r}
synStoreMany(
  c(
    "classification/okl_eurofins_partition_index.csv",
    "classification/okl_eurofins_partition_index_max.csv",
    "classification/okl_eurofins_catds.csv",
    "classification/okl_eurofins_catds_index_max.csv",
    "classification/okl_eurofins_catds_censored.csv",
    "classification/okl_eurofins_catds_index_max_censored.csv",
    "classification/okl_eurofins_chemical_probes_single_target_catds.csv",
    "classification/okl_eurofins_chemical_probes_by_gene_catds.csv"
  ),
  parentId = "syn18508401",
  used = unname(inputs),
  executed = "https://github.com/labsyspharm/okl-analysis/blob/main/okl_selectivity.Rmd",
  forceVersion = FALSE
)

```

```{r}
klaeger_partition_index_max %>%
  ggplot(aes(partition_index_max)) +
    stat_ecdf() +
    lims(x = c(0, 1))

shared_targets_klaeger_okl <- intersect(
  klaeger_ic50s$Name,
  pseudo_ic50$hgnc_symbol
)

kd_klaeger_okl <- bind_rows(
  pseudo_kd %>%
    filter(
      hgnc_symbol %in% shared_targets_klaeger_okl,
      !exclude_target,
      classification != "discordant"
    ) %>%
    transmute(
      lspci_id, hgnc_symbol, name, dataset,
      pseudo_ic50, pseudo_ic50_relation, pseudo_ic50_concordant,
      source = "okl"
    ),
  klaeger_ic50s %>%
    filter(
      Name %in% shared_targets_klaeger_okl
    ) %>%
    transmute(
      lspci_id, hgnc_symbol = Name, name = Compound,
      dataset = "klaeger",
      pseudo_ic50 = IC50, pseudo_ic50_relation = IC50_relation,
      pseudo_ic50_concordant = "concordant",
      source = "klaeger"
    )
) %>%
  mutate(
    source_dataset = paste(source, dataset, sep = "_")
  )

partition_index_klaeger_okl <- ic50_klaeger_okl %>%
  # filter(pseudo_ic50_relation == "=") %>%
  group_by(source, dataset, source_dataset, lspci_id) %>%
  # filter(n() > 50) %>%
  mutate(
    partition_index = (1 / pseudo_ic50) / sum(1 / pseudo_ic50, na.rm = TRUE)
  ) %>%
  ungroup()

partition_index_max_klaeger_okl <- partition_index_klaeger_okl %>%
  group_by(source, dataset, source_dataset, lspci_id) %>%
  summarize(
    partition_index_max = max(partition_index, na.rm = TRUE),
    .groups = "drop"
  )

partition_index_max_klaeger_okl %>%
  ggplot(aes(partition_index_max, color = source_dataset)) +
    stat_ecdf() +
    lims(x = c(0, 1))

p <- partition_index_max_klaeger_okl %>%
  filter(
    lspci_id %in% {
      group_by(., lspci_id) %>%
      filter(uniqueN(source) == 2) %>%
      pull(lspci_id)
    }
  ) %>%
  ggplot(aes(partition_index_max, color = source_dataset)) +
    stat_ecdf() +
    lims(x = c(0, 1)) +
    labs(
      x = "Partition index",
      y = "Cumulative fraction of compounds",
      title = "Compounds and targets shared between Klaeger and OKL",
      color = "Dataset"
    )

ggsave(
  file.path("plots", "partition_index_klaeger_okl_shared_compounds_and_targets_ecdf.pdf"),
  p, width = 6, height = 4
)

p <- partition_index_max_klaeger_okl %>%
  filter(
    lspci_id %in% {
      group_by(., lspci_id) %>%
      filter(uniqueN(source) == 2) %>%
      pull(lspci_id)
    }
  ) %>%
  select(-source, -dataset) %>%
  pivot_wider(names_from = source_dataset, values_from = partition_index_max) %>%
  ggplot(aes(klaeger_klaeger, okl_original_repeat_replaced)) +
    geom_point(shape = 16, alpha = 0.5) +
    geom_smooth(method = "lm") +
    lims(x = c(0, 1), y = c(0, 1)) +
    labs(x = "Partition index Klaeger", y = "Partition index OKL (repeat)")


ggsave(
  file.path("plots", "partition_index_klaeger_vs_okl_shared_compounds_and_targets.pdf"),
  p, width = 5, height = 5
)

partition_index_max_klaeger_okl %>%
  filter(
    lspci_id %in% {
      group_by(., lspci_id) %>%
      filter(uniqueN(source) == 2) %>%
      pull(lspci_id)
    }
  ) %>%
  select(-source, -dataset) %>%
  pivot_wider(names_from = source_dataset, values_from = partition_index_max) %>%
  {cor.test(.$klaeger_klaeger, .$okl_original_repeat_replaced)}


partition_index_max_klaeger_okl %>%
  filter(
    lspci_id %in% {
      group_by(., lspci_id) %>%
      filter(uniqueN(source) == 2) %>%
      pull(lspci_id)
    }
  ) %>%
  ggplot(aes(partition_index_max, fill = source_dataset)) +
    geom_bar(stat = "bin", color = "black", alpha = 0.5, position = "identity") +
    lims(x = c(0, 1))


```

